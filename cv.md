...
```
        ...
        init: function () {
            this.currentLink();
            this.makeStickyLinksBox();
        },

        currentLink: function () {
            const linksWrap = document.getElementById('links-box');
            let currentUrl = window.location.href;
            currentUrl = currentUrl.replace(/\/$/, '');
            if (linksWrap) {
                let links = linksWrap.querySelectorAll('.link-item');
                links.forEach(link => {
        ...
        makeStickyLinksBox: function () {
            const header = document.querySelector('header');
            const linksBox = document.getElementById('links-box');
            const placeholder = document.createElement('div');

            if (linksBox) {
                placeholder.style.height = `${linksBox.offsetHeight}px`;
                placeholder.style.display = 'none';

                linksBox.parentNode.insertBefore(placeholder, linksBox);
                const linksBoxOffsetTop = linksBox.offsetTop;
                const headerHeight = header.offsetHeight;

                window.addEventListener('scroll', function () {
                    const scrollY = window.scrollY;

                    if (scrollY >= linksBoxOffsetTop - headerHeight) {
                        linksBox.classList.add('sticky');
                        linksBox.style.top = `${headerHeight}px`;
                        placeholder.style.display = 'block';
                    } else {
                        linksBox.classList.remove('sticky');
                        linksBox.style.top = 'initial';
                        placeholder.style.display = 'none';
                    }
                });
            }
...
### 1 year 3 months
**Uco development**  
Israel, [uco.co.il](https://www.uco.co.il)

****
1. Website development including CMS WordPress, native PHP, and using the Laravel framework.
2. Integration with various delivery APIs and payment systems.
3. Developing themes and plugins, integrating API.
4. Website administration and hosting management.
...
### 3 months
**Enter2Web**  
Israel, [enter2web.co.il](https://enter2web.co.il)

****
1. Website development including CMS WordPress, native PHP, and using the Laravel framework.
2. Developing themes and plugins, integrating and working with the Woocommerce plugin API.
3. Proficiency in Figma and Photoshop for implementing designs.
4. Website administration and hosting management.
